  ...
  <svg :width="1000" :height="channelNames.length*50+50">
    <g transform="translate(250, 10)">
      <template v-for="(process, processindex) in stackedData">
        <rect v-for="(channel, channelindex) in process" :height="yScale.bandwidth()" :x="xScale(channel[0])" :y="yScale(channelNames[channelindex])" :width="xScale(channel[1])-xScale(channel[0])" :fill="color(processNames[processindex])" :class="{ isnothighlighted: !processisHighlighted(processindex) }" @mouseover="highlight(processindex)" @mouseleave="unhighlight"/>
      </template>
      <path fill="none" stroke="#000" 
            :d="pathStringX"></path>
      <text v-for="tick in parseInt(ticks)" :y="yScale(channelNames[channelNames.length-1])+barHeight+xlabeloffset" :x="xScale((tick-1)*20)" dominant-baseline="middle" text-anchor="middle">{{(tick-1)*20}}</text>
      ...
      <path fill="none" stroke="#000"
            :d="pathStringY"></path>
      <text v-for="channel in channelNames" :x="-10" :y="yScale(channel)+barHeight/2" dominant-baseline="middle" text-anchor="end">{{channel}}</text>
      <template v-for="(process, processindex) in stackedData">
