<script setup lang="ts">
    ...
    const files = ref()
    const button_is_disabled = computed(() => { return hepdata_id.value === '' })
...
<template>
    <div id="workspaceloader">
        ...
        <q-file multiple accept='.json'
            ref="file_input"
            @rejected="on_rejected"
            v-model="files" 
            @update:model-value="storeid_store.load_workspaces_from_local_files(files)" 
            style="visibility: hidden; width: 0; height: 0">
        </q-file>
        <q-btn icon="cloud_upload" @click="get_file" label="Read local file"></q-btn>
        <span style="margin: 1em">or</span>
        <div class="row">
            <q-input filled bottom-slots label="Enter HEPdata ID" type="text" v-model="hepdata_id" hide-bottom-space style="margin-right: 1em"/>
            <q-btn :disabled="button_is_disabled" @click="storeid_store.load_workspaces_from_HEPdata(hepdata_id)">Read from HEPdata</q-btn>
        </div>
    </div>
</template>
